...
struct vm_info* lookup_vm_info_by_inode(unsigned long inode){
	...
	while(vm_info_ptr != NULL){
		if (vm_info_ptr->file_inode == inode){
			return vm_info_ptr;
		}
...
static struct entry *update_hotspot_queue(struct smq_policy *mq, dm_oblock_t b, struct vm_info* vm_info_ptr)
{
	unsigned hi;
	...
	if (e) {
		...
		if (e) {
			e->oblock = hb;
			e->vm_info_ptr = vm_info_ptr;
			printk(KERN_INFO "[CS695.1.4] dm-cache-smq New entry created. e->vm_info->file_inode: %lu", e->vm_info->file_inode);
			q_push(&mq->hotspot, e);
		    ...
		    struct policy_work **work, bool *background_work, unsigned long inode)
{
	...
	struct vm_info* vm_info_ptr = NULL;

	*background_work = false;
	
	/*CS695*/
	if(inode>0){
		printk(KERN_INFO "[CS695.1.4] dm-cache-smq inode: %lu", inode);
		vm_info_ptr = lookup_vm_info_by_inode(inode);
		if (!vm_info_ptr){
			printk(KERN_INFO "[CS695.1.4] dm-cache-smq inode %lu not a vm file. Returning to dm-cache-target", inode);
			return -ENOENT;
		}
	}
	/*End CS695*/
	...
	if (e) {
		...
		printk(KERN_INFO "[CS695.1.4] dm-cache-smq entry found. e->vm_info->file_inode: %lu", e->vm_info->file_inode);
		...
		*cblock = infer_cblock(mq, e);
		 ...
		 */
		hs_e = update_hotspot_queue(mq, oblock, vm_info_ptr);
...
static int smq_lookup(struct dm_cache_policy *p, dm_oblock_t oblock, dm_cblock_t *cblock,
		      int data_dir, bool fast_copy,
		      bool *background_work, unsigned long inode)
{
	int r;
	unsigned long flags;
	struct smq_policy *mq = to_smq_policy(p);
	// if(inode>0){
	// 	printk(KERN_INFO "[CS695.1.4] dm-cache-smq smq_lookup inode : %lu", inode);
	// }
	spin_lock_irqsave(&mq->lock, flags);
	r = __lookup(mq, oblock, cblock,
		     data_dir, fast_copy,
		     NULL, background_work, inode);
...
static int smq_lookup_with_work(struct dm_cache_policy *p,
				...
				int data_dir, bool fast_copy,
				struct policy_work **work, unsigned long inode)
{
	int r;
	...
	struct smq_policy *mq = to_smq_policy(p);
	// if(inode>0){
	// 	printk(KERN_INFO "[CS695.1.4] dm-cache-smq smq_lookup_with_work inode : %lu", inode);
	// }
	spin_lock_irqsave(&mq->lock, flags);
...
static void smq_tick(struct dm_cache_policy *p, bool can_block)
{
	...
	end_cache_period(mq);
	spin_unlock_irqrestore(&mq->lock, flags);
	// printk(KERN_INFO "[CS695] Tick tock\n");
}
...
static int __init smq_init(void)
{
	...
	vi->next = NULL;
	vm_info_list = vi;
	// printk(KERN_INFO "[CS695] This is init of dm-cache-policy-smq after modules_install\n");
...
MODULE_AUTHOR("Joe Thornber <dm-devel@redhat.com>, Shinjan Mitra");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("smq cache policy modified for VM");
