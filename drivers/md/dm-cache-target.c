...
static int map_bio(struct cache *cache, struct bio *bio, dm_oblock_t block,
		   bool *commit_needed, unsigned long inode)
{
	int r, data_dir;
	...
	*commit_needed = false;
	...
	if (optimisable_bio(cache, bio, block)) {
		...
		// if(inode>0){
		// 	printk(KERN_INFO "[CS695.1.4] dm-cache-target inode optimisable bio : %lu", inode);
		// }
		r = policy_lookup_with_work(cache->policy, block, &cblock, data_dir, true, &op, inode);
		...
		// if(inode>0){
		// 	printk(KERN_INFO "[CS695.1.4] dm-cache-target inode non optimisable bio : %lu", inode);
		// }
		r = policy_lookup(cache->policy, block, &cblock, data_dir, false, &background_queued, inode);
		if (unlikely(r && r != -ENOENT)) {
...
{
	bool commit_needed;
	unsigned long inode = get_ino(bio);
	if (map_bio(cache, bio, get_bio_block(cache, bio), &commit_needed, inode) == DM_MAPIO_REMAPPED)
		generic_make_request(bio);
...
static int cache_map(struct dm_target *ti, struct bio *bio)
{
	...
	if (unlikely(from_oblock(block) >= from_oblock(cache->origin_blocks))) {
		...
		accounted_begin(cache, bio);
		// if(inode>0){
		// 	printk(KERN_INFO "[CS695.1.4] dm-cache-target inode unlikely from oblock  : %lu", inode);
		// }
		return DM_MAPIO_REMAPPED;
	}
	...
	if (discard_or_flush(bio)) {
		// if(inode>0){
		// 	printk(KERN_INFO "[CS695.1.4] dm-cache-target inode deffered : %lu", inode);
		// }
		defer_bio(cache, bio);
		return DM_MAPIO_SUBMITTED;
	}
	// if(inode>0){
	// 	printk(KERN_INFO "[CS695.1.4] dm-cache-target inode map_bio: %lu", inode);
	// }
	r = map_bio(cache, bio, block, &commit_needed, inode);
	if (commit_needed)
		schedule_commit(&cache->committer);
