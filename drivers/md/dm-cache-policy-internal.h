...
static inline int policy_lookup(struct dm_cache_policy *p, dm_oblock_t oblock, dm_cblock_t *cblock,
				int data_dir, bool fast_copy, bool *background_queued, unsigned long inode)
{
	return p->lookup(p, oblock, cblock, data_dir, fast_copy, background_queued, inode);
}
					  ...
					  struct policy_work **work, unsigned long inode)
{
	if (!p->lookup_with_work) {
		*work = NULL;
		return p->lookup(p, oblock, cblock, data_dir, fast_copy, NULL, inode);
	}

	return p->lookup_with_work(p, oblock, cblock, data_dir, fast_copy, work, inode);
}
